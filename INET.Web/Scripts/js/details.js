$(document).ready(function () {
    ...
    $('#approve-all-solicitudes-group').click(function (e) {
        triggerSolicitudeGroupUpdateAll('approve-all');
        e.preventDefault();
        return false;
    });

    $('#reject-all-solicitudes-group').click(function (e) {
        triggerSolicitudeGroupUpdateAll('reject-all');
        e.preventDefault();
        return false;
    });
    ...
    $("#SolicitudeDTO_ExpenditureTypeId").change(function () {
            ...
            // Busco las especializaciones para ese CUE
          $.get(BaseSiteURL + "/Plan/ListExpenditureObjectTypes", {
                improvementPlanId: $("#Id").val(),
              expenditureTypeId: $('#SolicitudeDTO_ExpenditureTypeId').val()
            }).done(function (data) {
                var prev_selected = $("#SolicitudeDTO_ExpenditureObjectTypeId").val();
                $("#SolicitudeDTO_ExpenditureObjectTypeId").empty();
                $("<option>").attr("value", "").text("Seleccionar...").appendTo("#SolicitudeDTO_ExpenditureObjectTypeId");

                $.each(data, function (i, item) {
                    $("<option>").attr("value", item.Key).text(item.Value).prop('selected', prev_selected == item.Key).appendTo("#SolicitudeDTO_ExpenditureObjectTypeId");
                });
            });
    ...
    $("#SolicitudeDTO_Line_22_Id").change(function () {
        // Si el plan se encuentra en evaluacion, y estoy cargando un solicitado nuevo, entonces muestro el número de expediente correspondiente a esa línea
        if ($("#OriginalStatusId").val() == "3" && $("#SolicitudeDTO_LineId :selected").val() != "") {
            $("#SolicitudeDTO_FileNumber").removeClass("input-validation-error");
            $.get(BaseSiteURL + "/Plan/GetFileNumberForSolicitude", {
    ...
    function getFileNumber() {
        ...
        if ($("#OriginalStatusId").val() == "3" && $("#SolicitudeDTO_LineId :selected").val() != "") {
            $("#SolicitudeDTO_FileNumber").removeClass("input-validation-error");
            $.get(BaseSiteURL + "/Plan/GetFileNumberForSolicitude_22", {
                improvementPlanId: $("#Id").val()
    ...
    $("#SolicitudeDTO_CUE").change(function () {
        // Si el plan se encuentra en evaluacion, y estoy cargando un solicitado nuevo, entonces muestro el número de expediente correspondiente a esa línea
      //  getFileNumber();
    });
    ...
    $("#nuevo-rendicion").click(function () {
           
       
        $("#new-account").dialog("open");
    });

    //$('.account-action-edit').click(function () {
    //    var $source = $(this);

    //    loadAccount($source.attr("data-AccountRenderingId"), $source);
    //});
    ...
    $(".improvementPlan-related-line").click(function () {
        $("#new-related-line")        
            .dialog("open");
    });
    ...
    $("#delete-confirm-related").dialog({
        ...
        buttons: {
            "Eliminar": function () {
                if (!canSubmitForm(getDialogFirstButton("#delete-confirm-related")))
                    return false;
                showLoading();
                $.post(BaseSiteURL + "/Plan/DeleteRelatedLine", {
                 
                    Id: $(this).attr("data-relatedLine-Id")
                }).done(function (data) {
                  
                    hideLoading();
                   
                    $("#delete-confirm-related").dialog("close");
                }).fail(function () {
                    showMessage("#msg-plan", "Ocurrió un error al eliminar el solicitado.", "error", true);
                    hideLoading();
                });
    ...
    $("#new-related-line").dialog({
        ...
        modal: true,
        open: function () {
            $("#frmRelatedLine").validate().resetForm();
            $("#RelatedLineId").val("");
        
            $("#msg-related-line").empty();
        },
        buttons: {
            "Guardar": function () {
              
                if ($("#frmRelatedLine").valid()) {
                    if (!canSubmitForm(getDialogFirstButton("#new-related-line")))
                        return false;

                    showLoading();
                    $.post(BaseSiteURL + "/Plan/SaveRelatedLine", {
                      
                        ImprovementPlanId: $("#Id").val(),
                        Line22Id: $("#RelatedLineId").val()
                       
                    }).done(function (data) {
                        
                        
                        $(".related-lines").html(data);
                              // bindIncidencesEvents();
                          
                      
                        

                        $("#new-related-line").dialog("close");
                        hideLoading();
                        showMessage("#msg-plan", "La Linea Relacionada se guardó con éxito", "confirmation", true);

                    }).fail(function (request, status, error) {
                        showMessage("#msg-related-line", "Ocurrió un error al guardar la linea", "error");
                       // hideLoading();
                    }).error( function (request, status, error) {
                        alert(request.responseText);
                    });
                } else {
                    showMessage("#msg-related-line", "Por favor, complete todos los campos obligatorios", "error");
                }
            },
        }
    });
    ...
    $("#import-spread-related").dialog({
        autoOpen: false,
        resizable: false,
        width: 500,
        modal: true,
        open: function () {
            $("#msg-import-related").empty();
            $("#frmImportRelatedPlanesSolicitudes").validate().resetForm();
        },
        buttons: {
            "Cargar": function () {
                if ($("#frmImportRelatedPlanesSolicitudes").valid()) {
                    if (!canSubmitForm(getDialogFirstButton("#import-spread-related")))
                        return false;
                    showLoading();
                    // Submiteo el formulario con un timeout, porque sino no da tiempo a mostrar el icono de loading en FF
                    setTimeout(importRelatedSolicitudes, 500);
                } else {
                    showMessage("#msg-import-related", "Por favor, complete todos los campos obligatorios", "error", true);
                }
            },
            "Cancelar": function () {
                $(this).dialog("close");
    ...
    $("#import-related").click(function () {
        $("#import-spread-related").dialog("open");
    });
    ...
    $("#new-solicitation").dialog({
        ...
        open: function () {
            ...
            // si ya tiene Expedientes, traerlos!
            getFileNumber();
            ...
            //$("#SolicitudeDTO_LineId").attr("disabled", "disabled");
            //$("#SolicitudeDTO_Line22_Id").attr("disabled", "disabled");
            ...
            "Guardar": function () {
                ...
                if (validSolicitude()) {
                    ...
                    $.post(BaseSiteURL + "/Plan/SaveSolicitude", {
                        ...
                        LineId: $("#LineId").val(),
                        FieldId: $("#FieldId").val(),
                        SubFieldId: $("#SubFieldId").val(),
                        //Line_22_Id: $("#SolicitudeDTO_Line22_Id :selected").val(),
                        Line_22_Id: $("#Line22Id").val(),
                        ...
                        ExpenditureObjectTypeId: $("#SolicitudeDTO_ExpenditureObjectTypeId :selected").val(),
    ...
    $("#new-account").dialog({
        ...
        close: function () {
            //enableSolicitdeForm();SaveAccount
            //clearSolicitudeForm();
           // $("#SolicitudeDTO_SolicitudeId").val("0");
           // $(".ui-dialog-buttonset").find("button :first").show(); // Por defecto, vuelvo a dejar siempre visible el botón de guardar            
        },
        buttons: {
            "Guardar": function () {
                $("#frmAccount").validate().resetForm();
                    ...
                    if (!canSubmitForm(getDialogFirstButton("#new-account"))) {
                        return false;
                    }
                    showLoading();
                    $.post(BaseSiteURL + "/Plan/SaveAccount", {
                        //dto:$("#AccountRenderingDTO").val(),
                        improvementPlanId: $("#Id").val(),
                        id: $("#AccountRenderingDTO_id").val(),
                        aprovedAmount: $("#AccountRenderingDTO_aprovedAmount").val(),
                        day: $("#AccountRenderingDTO_DateDay").val(),
                        month: $("#AccountRenderingDTO_DateMonth").val(),
                        year: $("#AccountRenderingDTO_DateYear").val(),
                        expenditureObjectTypeId: $("#AccountRenderingDTO_expenditureObjectTypeId").val(),
                        number: $("#AccountRenderingDTO_number").val(),
                        dictumId: $("#AccountRenderingDTO_dictumId").val(),
                        rejectedAmount: $("#AccountRenderingDTO_rejectedAmount").val(),
                        comments: $("#AccountRenderingDTO_comment").val(),
                        ...
                        if (data != "") {
                            ...
                            // reloadLinesCombo();
                          //  $("#list-accounts").empty();
                            $("#list-accounts").html(data);
                            $("#new-account").dialog("close");
                            showMessage("#msg-account", "La incidencia se guardó con éxito", "confirmation", true);

                            hideLoading();
                            location.reload();
                        } else {
                            showMessage("#msg-account", "El CUE ingresado o la línea seleccionada no son válidos.", "attention");
                            hideLoading();
                        }

                    }).fail(function (xhr, textStatus, errorThrown) {
                        var responseError = xhr.responseText.substr(xhr.responseText.indexOf("<title>") + 7, xhr.responseText.indexOf("</title>") - xhr.responseText.indexOf("<title>") - 7);
                        showMessage("#msg-account", responseError, "error");
                        enableSubmitButtons();
                        hideLoading();
                    });
    ...
    $("#delete-confirm-account").dialog({
        ...
        buttons: {
            "Eliminar": function () {
                if (!canSubmitForm(getDialogFirstButton("#delete-confirm-account")))
                    return false;
                showLoading();
                $.post(BaseSiteURL + "/Plan/DeleteAccount", {
                    
                    AccountId: $(this).attr("data-AccountRenderingId")
                }).done(function (data) {
                    if (data != "") {
                        location.reload();
                        showMessage("#msg-plan", "La rendicion  se eliminó con éxito.", "confirmation", true);
                    } else {
                        showMessage("#msg-plan", "No es posible eliminar la rendicion", "error", true);
                    }
                    hideLoading();
                    enableSubmitButtons();
                    $("#delete-confirm-account").dialog("close");
                }).fail(function () {
                    showMessage("#msg-plan", "Ocurrió un error al eliminar la rendicion.", "error", true);
                    hideLoading();
                });
            },
            "Cancelar": function () {
    ...
    $("#AccountRenderingDTO_dictumId").change(function () {


         var dictumId = $(this).val();
        // Busco las especializaciones para ese CUE
        $.get(BaseSiteURL + "/Plan/AccountDictumAmount", {
            DictumId: dictumId
           
        }).done(function (data) {
            $("#AccountRenderingDTO_aprovedAmount").val(data.amount);
        });
    ...
    account_table = $('#list-accounts').dataTable({
        "bPaginate": false,
        "bLengthChange": false,
        "bFilter": true,
        "bSort": true,
    ...
    $('#list-accounts').on('click', '.account-action-edit', loadAccountAction);
    $('#list-accounts').on('click', '.account-action-delete', deleteAccount);
...
function importRelatedSolicitudes() {
    showLoading();
    $("#frmImportRelatedPlanesSolicitudes").submit();
}
...
function clearAccountForm() {
    ...
    $("#msg-account").empty();
    // Limpio valores
    $("#AccountRenderingDTO_aprovedAmount").val("");
    $("#AccountRenderingDTO_expenditureObjectTypeId").val("");
    $("#AccountRenderingDTO_dictumId").val($("#AccountRenderingDTO_dictumId option:first").val());
    $("#AccountRenderingDTO_expenditureObjectTypeId").val($("#AccountRenderingDTO_expenditureObjectTypeId option:first").val());
    $("#AccountRenderingDTO_number").val("");
...
function loadStatusAndSpecializations() {
    ...
    $.get(BaseSiteURL + "/Plan/LoadSolicitudeCombos", {
        ...
        // Cargo Combo con Objeto de Gasto

        $.get(BaseSiteURL + "/Plan/ListExpenditureObjectTypesBySol", {
            ...
            $.each(data, function (i, item) {
                $("<option>").attr("value", item.Key).text(item.Value).appendTo("#SolicitudeDTO_ExpenditureObjectTypeId");
            });
        });
...
function loadSolicitude(id, $source) {
    ...
    $.get(BaseSiteURL + "/Plan/GetSolicitude", {
        ...
        $("#SolicitudeDTO_ExpenditureObjectTypeId").val(data.ExpenditureObjectTypeId);
        ...
        data.RequestedAmount = Number(data.RequestedAmount).toFixed(2).replace('.', ',');
...
function loadAccountAction()
{
    var id = ' ' + $(this).context.attributes.title.nodeValue;
    loadAccount(id, $(this));
};
function loadAccount(id, $source) {
    showLoading();
    $.get(BaseSiteURL + "/Plan/GetAccount", {
        Id: id
    }).done(function (data) {
        // Limpio valores
      
        $("#AccountRenderingDTO_id").val(id);
        $("#AccountRenderingDTO_expenditureObjectTypeId").val(data.expenditureObjectTypeId);
        $("#AccountRenderingDTO_dictumId").val(data.dictumId);

       // $("#AccountRenderingDTO_dictumId").val($("#AccountRenderingDTO_dictumId option:first").val());
       // $("#AccountRenderingDTO_expenditureObjectTypeId").val($("#AccountRenderingDTO_expenditureObjectTypeId option:first").val());

        $("#AccountRenderingDTO_improvementPlanId").val(data.improvementPlanId);
        $("#AccountRenderingDTO_number").val(data.number);
        $("#AccountRenderingDTO_DateDay").val(data.DateDay);
        $("#AccountRenderingDTO_DateMonth").val(data.DateMonth);
        $("#AccountRenderingDTO_DateYear").val(data.DateYear);
        $("#AccountRenderingDTO_aprovedAmount").val(data.aprovedAmount);
        $("#AccountRenderingDTO_rejectedAmount").val(data.rejectedAmount);
        $("#AccountRenderingDTO_comment").val(data.comment);
...
function enableSolicitdeForm() {
    ...
    //SIA 18-07-22 -combo d linea y sublinea
...
function deleteRelatedLine() {
    $("#delete-confirm-related")
        .attr("data-relatedLineId", $(this).attr("data-relatedLineId"))
        .dialog("open");
}
function deleteAccount() {
    $("#delete-confirm-account")
        .attr("data-AccountRenderingId", $(this).attr("data-AccountRenderingId"))
        .dialog("open");
}
...
function reloadLinesCombo() {
    var $source = $(this);
    showLoading();
    $.get(BaseSiteURL + "/Plan/LoadLineasCombos", {
        improvementPlanId: $("#Id").val(),
        
    }).done(function (data) {
        // Cargo el combo de Lineas para ese solicitado
        $("#SolicitudeDTO_LineId").empty();
        $("<option>").attr("value", "").text("Seleccionar...").appendTo("#SolicitudeDTO_LineId");

        $.each(data.Lines, function (i, item) {
            $("<option>").attr("value", item.Key).text(item.Value).appendTo("#SolicitudeDTO_LineId");
        });
...
function triggerSolicitudeGroupUpdateAll(action) {
